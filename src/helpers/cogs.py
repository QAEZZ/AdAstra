...
class CogHelper:
    ...
    async def load(self, cog: str) -> None:
        ...
        embed: discord.Embed = discord.Embed(title="Result", color=EMBED_COLOR)
            ...
            for filename in os.listdir(f"cogs/{cog_folder}"):
                if filename.endswith(".py"):
                    try:
                        ...
                        embed.add_field(
                            name="✅", value=f"```{cog} loaded.```", inline=False
                        )
                    except Exception as e:
                        embed.add_field(name="❌", value=f"```{e}```", inline=False)

                    time.sleep(0.75)  # ratelimit
            ...
            for filename in os.listdir(f"cogs/{cog_folder}"):
                if filename.endswith(".py"):
                    try:
                        ...
                        embed.add_field(
                            name="✅", value=f"```{cog} unloaded.```", inline=False
                        )
                    except Exception as e:
                        embed.add_field(name="❌", value=f"```{e}```", inline=False)
            ...
            for filename in os.listdir(f"cogs/{cog_folder}"):
                if filename.endswith(".py"):
                    try:
                        ...
                        await self.client.load_extension(cog)
                        embed.add_field(
                            name="✅", value=f"```{cog} reloaded.```", inline=False
                        )
                    except Exception as e:
                        embed.add_field(name="❌", value=f"```{e}```", inline=False)
